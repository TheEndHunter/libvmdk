...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libvmdk.sln 2>&1" | %{ "$_" }
	}
}
$VSSolutionFile = "${VSSolutionPath}\libvmdk.sln"
